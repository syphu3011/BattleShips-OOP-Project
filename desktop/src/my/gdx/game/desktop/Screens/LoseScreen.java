...
public class LoseScreen implements Screen,ScreenInterface {
    private Stage stage;
    private Game game;
    ...
    public void animateStarting() {
        ...
        int count = 2;
        Label lblLoser;
        //đưa dòng chữ thua cuộc vào
        lblLoser = new Label("YOU",new Label.LabelStyle(font,Color.ORANGE));
        lblLoser.setFontScale(3);
        lblLoser.setPosition(-200, stage.getHeight()-200);
        lblLoser.setAlignment(Align.center);
        lblLoser.addAction(Actions.sequence(Actions.delay((TIMEWAIT) * count),Actions.moveTo(stage.getWidth()/2 - 150,lblLoser.getY(),TIMEWAIT)));
        backGround.addActor(lblLoser);

        lblLoser = new Label("LOSE!",new Label.LabelStyle(font,Color.ORANGE));
        lblLoser.setFontScale(3);
        lblLoser.setPosition(stage.getWidth() + 200, stage.getHeight()-200);
        lblLoser.setAlignment(Align.center);
        lblLoser.addAction(Actions.sequence(Actions.delay((TIMEWAIT) * count++),Actions.moveTo(stage.getWidth()/2 + 100,lblLoser.getY(),TIMEWAIT)));
        backGround.addActor(lblLoser);
        //đưa prize vào
        lblLoser = new Label("PRIZE: "+prize,new Label.LabelStyle(font,Color.RED));
        lblLoser.setFontScale(2);
        lblLoser.setPosition(-250, stage.getHeight()/2 + 50);
        lblLoser.addAction(Actions.sequence(Actions.delay((TIMEWAIT-0.2f) * count++),Actions.moveTo(stage.getWidth()/2-100,lblLoser.getY(),TIMEWAIT)));
        backGround.addActor(lblLoser);
        //đưa số tiền vào
        lblLoser = new Label("GOLD: "+Asset.gold,new Label.LabelStyle(font,Color.RED));
        lblLoser.setFontScale(2);
        lblLoser.setPosition(stage.getWidth() + 250, stage.getHeight()/2 - 50);
        lblLoser.addAction(Actions.sequence(Actions.delay((TIMEWAIT-0.2f) * count++),Actions.moveTo(stage.getWidth()/2-100,lblLoser.getY(),TIMEWAIT)));
        backGround.addActor(lblLoser);
    }
    ...
    public void setItem() {
        // TODO Auto-generated method stub
        //thêm button
        Size btnSize = new Size(stage.getWidth() / 6, stage.getWidth() / 6 * RATIO);
